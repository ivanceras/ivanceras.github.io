...
var ASM_CONSTS = [function($0) { Module.STDWEB.tmp = Module.STDWEB.to_js( $0 ); },
 ...
 function($0, $1, $2, $3) { Module.STDWEB.from_js($0, (function(){var spongedown_parse = Module.STDWEB.to_js($1); var parse_bob = Module.STDWEB.to_js($2); var parse_csv = Module.STDWEB.to_js($3); self.addEventListener ("message" , function (e){var data = e.data ; switch (data.cmd){case "start" : self.postMessage ("WORKER STARTED: " + data.msg); break ; case "stop" : self.postMessage ("WORKER STOPPED: " + data.msg + ". (buttons will no longer work)"); self.close (); break ; case "spongedown_parse" : {let html = spongedown_parse (data.data); self.postMessage ({"caller" : "spongedown_parse" , "data" : html});}break ; case "parse_bob" : {let svg = parse_bob (data.data); self.postMessage ({"caller" : "parse_bob" , "data" : svg});}break ; case "parse_csv" : {let html = parse_csv (data.data); self.postMessage ({"caller" : "parse_csv" , "data" : html});}break ; default : self.postMessage ("Unknown command: " + data.msg);};}, false); spongedown_parse.drop (); parse_bob.drop (); parse_csv.drop ();})()); },
...
Module['wasmTableSize'] = 167936;

Module['wasmMaxTableSize'] = 167936;
